...
interface PosReportsContentProps {
  showTitle?: boolean;
}

export function PosReportsContent({ showTitle = true }: PosReportsContentProps) {
  const { user } = useAuth();
                    ...
                    <TableBody>
                      ...
                      <TableRow>
                        ...
                        <TableCell className="text-right">
                          {formatTablePrice(data.metrics.totalRevenue)}
                        </TableCell>
                        <TableCell className="text-right text-muted-foreground">
                          {data.metrics.totalOrders > 0
                            ? `${formatTablePrice(data.metrics.averageOrderValue)} avg`
                            : "No orders"}
                        </TableCell>
                      </TableRow>
                      <TableRow>
                          ...
                          ([dept, { count, total }]) => {
                            ...
                            return (
                              <TableRow key={dept}>
                                ...
                                <TableCell className="text-right">
                                  {formatTablePrice(Math.round(total / count))}
                                </TableCell>
                                <TableCell className="text-right">{percentage}%</TableCell>
                              </TableRow>
                            );
                      ...
                      {Array.from({ length: 24 }, (_, i) => {
                        ...
                        return (
                          <TableRow key={i}>
                            ...
                            <TableCell className="text-right">
                              {formatTablePrice(hourData.total)}
                            </TableCell>
                            <TableCell className="text-right">
                              {formatTablePrice(Math.round(hourData.total / hourData.count))}
                            </TableCell>
                            <TableCell className="text-right">{percentage}%</TableCell>
                          </TableRow>
                        );
