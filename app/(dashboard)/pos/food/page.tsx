...
export default function PosFoodPage() {
    ...
    const columns = useMemo<Column<FoodItem>[]>(
        () => [
            ...
            { key: "active", label: "Active", render: (v) => (v ? "Yes" : "No"), width: "w-24" },
            {
                ...
                render: (_v, item) => (
                    <div className="flex gap-2">
                        <button className="btn" onClick={(e) => { e.stopPropagation(); router.push(`/dashboard/pos/food/${item.id}`); }}>View</button>
                        <button className="btn" onClick={(e) => { e.stopPropagation(); router.push(`/dashboard/pos/food/${item.id}/edit`); }}>Edit</button>
                    </div>
                ),
                width: "w-36",
            },
            ...
            <div className="grid gap-4">
                <div className="flex items-center gap-4">
                    <div className="flex-1">
                        <TableSearchBar value={search} onChange={(v) => { setSearch(v); setPage(1); }} placeholder="Search food items..." isLoading={isLoading} />
                    </div>
                    <TableFilterBar
                ...
                {isLoading ? (
                    ...
                    <DataTable columns={columns} data={items} isLoading={isLoading} pagination={{ total, page, limit, onPageChange: setPage }} onRowClick={(item) => router.push(`/dashboard/pos/food/${item.id}`)} />
                )}
            </div>
        </div>
    );
